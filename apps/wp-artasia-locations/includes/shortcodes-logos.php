...
function artasia_get_logo_grid_item(WP_Post $post, string $variant): ?array
{
    ...
    return [
        ...
        'label'          => $post->post_type === 'artasia_partner'
            ? trim((string) get_post_meta($post->ID, 'artasia_partner_label', true))
            : '',
...
{
    $project_item_ids = [];
    if ($project_id && $post_type === 'artasia_supporter') {
        $project_item_ids = artasia_get_project_supporter_ids($project_id);
    } elseif ($project_id && $post_type === 'artasia_partner') {
        $project_item_ids = artasia_get_project_partner_ids($project_id);
    }
    $posts = get_posts([
        'post_type'      => $post_type,
        'posts_per_page' => -1,
    ...
    foreach ($posts as $post) {
        if (
            in_array($post_type, ['artasia_supporter', 'artasia_partner'], true)
            && $project_id
            && !in_array($post->ID, $project_item_ids, true)
        ) {
            continue;
        }
        ...
        if ($item !== null) {
            if (in_array($post_type, ['artasia_supporter', 'artasia_partner'], true) && $project_id) {
    ...
    if (in_array($post_type, ['artasia_supporter', 'artasia_partner'], true) && $project_id) {
        foreach ($project_item_ids as $item_id) {
...
function artasia_get_project_partner_ids(int $project_id): array
{
    $recognitions = get_posts([
        'post_type'      => 'artasia_recognition',
        'posts_per_page' => -1,
            ...
            <li class="artasia-logo-grid__item artasia-logo-grid__item--<?php echo esc_attr($item['variant']); ?>">
                <?php if ($item['website'] !== '') : ?>
                    <a class="artasia-logo-grid__link" href="<?php echo esc_url($item['website']); ?>" target="_blank" rel="noopener noreferrer">
                        <span class="artasia-logo-grid__content">
                            <?php if ($item['label'] !== '') : ?>
                                <span class="artasia-logo-grid__label"><?php echo esc_html($item['label']); ?></span>
                            <?php endif; ?>
