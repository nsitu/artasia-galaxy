...
- Look up each remaining asset's exact `source:drive:<id>` in Google Drive, then inspect its **immediate parent folder**. A name containing `process` or `final` in any casing qualifies, including `Process photos`, `Work in PROCESS`, `Postprocessing`, `Final`, `FINAL photos`, and `Finalized artwork`. Both matches receive the **Process** asset type, not a separate Final type.
- This backfill intentionally does not walk higher ancestors or infer activity/placement assignments. A file under `Process/Details` or `Final/Details` does not qualify unless its immediate parent name also contains `process` or `final`. This differs from new-import traversal, where descendants inherit a Process classification. The Final match applies only to this maintenance tool; placement auto-import's folder rules are unchanged.
